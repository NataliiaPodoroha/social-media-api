...
POSTS_URL = reverse("blog:posts-list")
POSTS_CREATE_URL = reverse("blog:posts-create")
...
def get_detail_post_url(post_id):
    return reverse("blog:posts-detail", args=[post_id])


def get_like_unlike_post_url(post_id):
    return reverse("blog:posts-like-unlike", args=[post_id])


def get_url_for_add_comment(post_id):
    return reverse("blog:comment-create", args=[post_id])


def get_detail_comment_url(post_id, comment_id):
    return reverse("blog:comment-detail", args=[post_id, comment_id])


def get_url_for_like_unlike_comment(post_id, comment_id):
    return reverse("blog:comment-like-unlike", args=[post_id, comment_id])
...
class AuthenticatedSocialMediaApiTests(TestCase):
    ...
    def test_like_unlike_comment(self):
        ...
        comment = Comment.objects.create(
            content="test content", post_id=post.id, author=self.user
        )

        like_count = Comment.objects.get(id=comment.id).likes.count()

        like_url = get_url_for_like_unlike_comment(
            post_id=post.id, comment_id=comment.id
        )
        self.client.post(like_url)
        comment_url = get_detail_comment_url(post_id=post.id, comment_id=comment.id)
        response = self.client.get(comment_url)

        like_count_after = len(response.data["likes"])
